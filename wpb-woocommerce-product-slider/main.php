 ...
 * Text Domain: wpb-woocommerce-product-slider
...
if (! defined('WPB_WPS_FREE_VERSION')) {
    define('WPB_WPS_FREE_VERSION', '2.4');
}
...
?>
        <div class="error">
            <p><?php esc_html_e('You can\'t activate the free version of WPB Product Slider for WooCommerce while you are using the premium one.', 'wpb-woocommerce-product-slider'); ?></p>
        </div>
<?php
    }
...
if (!function_exists('wpb_wps_free_activation_redirect')) {
    function wpb_wps_free_activation_redirect($plugin)
    {
        if ($plugin == plugin_basename(__FILE__)) {
            if (! get_option('wpb_wps_installed')) {
                update_option('wpb_wps_installed', time());
            }
            wp_safe_redirect(admin_url('admin.php?page=wpb-wps-settings'));
            exit;
        }
    }
}
...
if (!function_exists('wpb_wps_add_action_links')) {
    function wpb_wps_add_action_links($links)
    {
        ...
        $links[] = '<a href="' . esc_url(get_admin_url(null, 'admin.php?page=wpb-wps-settings')) . '">' . esc_html__('Settings', 'wpb-woocommerce-product-slider') . '</a>';
        $links[] = '<a href="https://wpbean.com/support/" target="_blank">' . esc_html__('Support', 'wpb-woocommerce-product-slider') . '</a>';
        $links[] = '<a style="color: #27ae60;font-weight: bold;" target="_blank" href="' . esc_url('https://wpbean.com/downloads/wpb-woocommerce-product-slider-pro/') . '">' . esc_html__('Upgrade to PRO!', 'wpb-woocommerce-product-slider') . '</a>';
...
function wpb_wps_pro_discount_admin_notice()
{
    ...
    if (!get_user_meta($user_id, 'wpb_wps_pro_discount_dismissed')) {
        printf('<div class="wpb-wps-discount-notice updated" style="padding: 30px 20px;border-left-color: #27ae60;border-left-width: 5px;margin-top: 20px;"><p style="font-size: 18px;line-height: 32px">%s <a target="_blank" href="%s">%s</a>! %s <b>%s</b></p><a href="%s">%s</a></div>', esc_html__('Get a 10% exclusive discount on the premium version of the', 'wpb-woocommerce-product-slider'), 'https://wpbean.com/downloads/wpb-woocommerce-product-slider-pro/', esc_html__('WPB WooCommerce Product Slider', 'wpb-woocommerce-product-slider'), esc_html__('Use discount code - ', 'wpb-woocommerce-product-slider'), '10PERCENTOFF', esc_url(add_query_arg('wpb-wps-pro-discount-admin-notice-dismissed', 'true')), esc_html__('Dismiss', 'wpb-woocommerce-product-slider'));
    }
}
...
function wpb_wps_free_plugin_init()
{
    ...
    require_once dirname(__FILE__) . '/inc/class-wpb-wps-review-notice.php';

    new WPB_WPS_Review_Notice();
}
